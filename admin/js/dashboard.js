...
async function loadRecentRoutes() {
    ...
    if (!data || data.length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" class="empty-text">No routes yet.</td></tr>';
        return;
    }
...
// ---- Delete Route (for accidental duplicates) ----
async function deleteRoute(routeId) {
    if (!confirm('Delete this route? This cannot be undone.')) return;

    try {
        // Delete in order: stop_products > route_stops > delivery_events > route
        ...
        // Get the GD numbers before deleting the route (to mark them available again)
